...
library(RTutor)
library(yaml)
setwd("~/Studium_Master/MASTERARBEIT/Arbeit/R/Problemsets/RTutorEcologicalFootprintOfPovertyAlleviationPS3")
...
#< info "Comment on deforestation data set"
Please note that this data set (*"data_deforestation.rds"*) deviates from the original data set provided by the authors (*"ecological_footprint_restat_deforestation_data.dta"*) in three aspects: First, the column describing the **percentage of area deforested** was **multiplied by** $100$ for the numbers to be interpreted in percentage and not as simple proportion. This makes the results consistent with those printed in the paper. Second, the **variable names** were changed into **more intuitive** ones using abbreviations based on the English language. Third, the variable which is described as *Ln(slope)* in the paper and given as *lnslope* in the original data set is described as slope in degrees (not logarithmized). This is because the variable description in the original data set is "Average slope" and the values contained only make sense if they are interpreted as degree values. *(Cf. paper, Tables 2-5; original data set  ("ecological_footprint_restat_deforestation_data.dta"))*
#>
...
* The command *group_by(data=..., variable=...)* "groups" the data according to the variable given as argument such that the surrounding command - in this case *mutate()* is computed for all observations with the same value of this variable. Please note that this command returns a different data format: a tibble. For more information see for example <https://cran.r-project.org/web/packages/tibble/vignettes/tibble.html>.
#>
...
*(Cf. [Weier, J. & D. Herring (2000)](http://earthobservatory.nasa.gov/Features/MeasuringVegetation/printall.php "http://earthobservatory.nasa.gov/Features/MeasuringVegetation/printall.php"))*
#>
...
This is exactly what the function *deltaMethod(object, vcov., g, constants)* from the package *'car'* does. *(Cf. [CRAN Reference manual for package 'car'](https://cran.r-project.org/web/packages/car/car.pdf "https://cran.r-project.org/web/packages/car/car.pdf") (2016, pp.45-48))* Here, the argument *object* is the vector of the Tobit coefficient estimates and the logarithmized standard error of $\epsilon$, the argument *vcov.* is the variance-covariance matrix of this vector. The argument *constants* are the further constants used in the formula *g* which is the marginal effect formula (written as a string) in our case.  
...
#>
```
#< info "'ggplot2' - geom_bar, coord_flip and theme"
...
The command *theme(axis.text = element_text(size = 15))* defines the font size of the axis labels.
#>
...
* [Stata manual, command ivtobit (2016)](http://www.stata.com/manuals13/rivtobit.pdf "http://www.stata.com/manuals13/rivtobit.pdf")  
* [Stata manual, command ivprobit (2016)](http://www.stata.com/manuals13/rivprobit.pdf "http://www.stata.com/manuals13/rivprobit.pdf").

If you are just interested in the estimation procedure, I recommend the last source to you. The algorithm on page 12 can be adapted to the algorithm for the IV Tobit two-step estimator by just replacing the probit estimation by a Tobit estimation. *(Cf. [Stata manual, command ivtobit (2016, p.10)](http://www.stata.com/manuals13/rivtobit.pdf "http://www.stata.com/manuals13/rivtobit.pdf"))*
...
$$u = V \rho + \epsilon \quad \text{with} \quad \epsilon \sim N(0,\tau_1^2)$$
...
#- Newey (1987)
#- Stata manual, command ivtobit (2016)
#- Stata manual, command ivprobit (2016, p.11-12)
...
#< info "Remark on the results from Newey_SB_ivtobit"
...
Please note that in the paper the authors state a number which deviates a bit from the one we have obtained here for the baseline probability of deforestation. *(Cf. paper, p.426)* However, as in the STATA code made available by the authors the according calculation is not included, I cannot tell where this (small) deviation comes from.
#>
...
#< info "Comment on household data set"
...
ggplot(d, aes(x=period, group=treatment, fill = treatment)) +
  geom_bar( width = 0.5, position = "dodge") +
  ylab("Frequency") +
  facet_wrap(~poor) + theme(axis.text = element_text(size = 15)) +
  theme(axis.title = element_text(size = 15)) +
  theme(legend.text = element_text(size = 15)) +
  theme(legend.title = element_text(size = 15)) +
  theme(strip.text = element_text(size=12))
...
* *summarize(data=..., column=...)* does the same as *mutate* with the difference that it only gives back the newly computed values. If combined with *group_by* the resulting tibble contains as many elements as there are unique values of the variable used for grouping.
#>
...
parts: 
  - question: 1. According to the model above, is there a chance of approximately measuring the true total treatment effect?
    ...
    success: Great! This is correct! 
    failure: Try again. 
  - question: 2. If you see a chance of measuring the effect approximately correctly, for which level of infrastructure does the model predict the measured average treatment effect to be closest to the true total treatment effect?
    choices:
...
The table at the end of this *'info' box* is a replicate of table 10 in the paper.
