...
library(RTutor)
check.problem.set('Ecological Footprint of Poverty Alleviation', ps.dir, ps.file, user.name=user.name, reset=FALSE)
...
*(Cf. paper; Skoufias (2005); [Prospera homepage (2016)](http://www.gob.mx/prospera "http://www.gob.mx/prospera"))*
...
info("Links to interesting websites") # Run this line (Strg-Enter) to show info
...
```{r "1_1__2",fig.height=3}
#Show the first six rows of data_deforest
```

info("Variable descriptions") # Run this line (Strg-Enter) to show info
...
* **Characteristics of the communities:** e.g. population numbers (*lnpop*), infrastructure quantity (*dens_road*)
...
info("Comment on deforestation data set") # Run this line (Strg-Enter) to show info
...
info("Remark on eligibility") # Run this line (Strg-Enter) to show info
...
info("Creating the bins of the plot - and shifting them") # Run this line (Strg-Enter) to show info
...
Further, the mean of the variable *treat* for each value of the variable *aux_bins_povind* - so for each auxiliary bin - is calculated and called *prop_treated*. This is done by the command *summarize* in combination with *group_by* which finally returns a tibble containing only the grouping variable *bins_povind* and the newly calculated column *prop_treated*. After having run the code, have a look at the shown first six rows of the returned tibble to get an idea about what the command has done.
...
head(dat)
```
info("Data manipulation with 'dplyr' - mutate, summarize and group_by") # Run this line (Strg-Enter) to show info
...
info("Measurement of deforestation") # Run this line (Strg-Enter) to show info
...
Now use the command *summarize(group_by())* from the package *'dplyr'* to create a tibble with two columns: One containing the grouping variable *aux_bins_povind* and one containing the mean of the variable *d* for all localities with the same value of *aux_bins_povind*. If you need some help for this task, have a look at *Exercise 1.1b* again. Call the newly calculated variable *prop_posdefor* and the summarized data set *dat_d*. Finally, show the first six rows of *dat_d*.
...
This works analogous to the illustration of the probability of deforestation: The variable of interest of which we will calculate the mean in each bin is now *pct_deforested* instead of *d*. To shorten it, the code for the respective data preparation and plotting is already given. Besides, the head of *dat_pct* is shown.  
*This corresponds to figure 3 on p.422 in the paper.*
```{r "1_2__7"}
...
info("Ordinary Least Squares") # Run this line (Strg-Enter) to show info
...
info("Displaying regression results: showreg") # Run this line (Strg-Enter) to show info
...
info("General technical remark") # Run this line (Strg-Enter) to show info
...
info("Linear Probability Model") # Run this line (Strg-Enter) to show info
...
In the following code chunk, we are going to estimate the LPM of *d* on the dummy variable *treat* by OLS. As we need to account for heteroskedasticity, we have to use robust standard errors. Alix-Garcia et al. use the kind of robust standard errors calculated by Stata: Huber-White standard errors (HC1). *(For further information on this, see the according help pages of STATA: [Stata manual, command regress (2016)](http://www.stata.com/manuals13/rregress.pdf "http://www.stata.com/manuals13/rregress.pdf") and [Stata manual, Chapter 20 (2016)](http://www.stata.com/manuals13/u20.pdf "http://www.stata.com/manuals13/u20.pdf") (subchapter 20.21))* We can quite easily replicate them using the arguments *robust=TRUE* and *robust.type="H1"* in the *showreg* command.  
...
info("Tobit Regressions in R") # Run this line (Strg-Enter) to show info
```{r "2_1__5"}
#Load package 'AER'
library(AER)
...
info("Displaying estimation results: stargazer and convert") # Run this line (Strg-Enter) to show info
...
```
info("Marginal Effects on E[y|x,y>0] and P(y>0|x)") # Run this line (Strg-Enter) to show info
info("margEff_tobit_dM_AER(model, data) - Introduction and Code") # Run this line (Strg-Enter) to show info
...
info("Key factors affecting deforestation") # Run this line (Strg-Enter) to show info
...
- - -
- - -
#! addon__quiz__Expected sign - poverty index
- - -
#! addon__quiz__Expected sign - population
- - -
#! addon__quiz__Expected sign - area
- - -
#! addon__quiz__Expected sign - forest baseline
- - -
#! addon__quiz__Expected sign - road density
- - -
#! addon__quiz__Expected sign - ecoregional controls
- - -
- - -
...
info("Remark on the answer to the third question") # Run this line (Strg-Enter) to show info
...
p_pctdefor_small
```
This graphics gives even less evidence for any jump at the cutoff. In contrary, the behavior of the dependent variable seems to be rather random.
...
info("Kernel regressions") # Run this line (Strg-Enter) to show info
...
info("Data manipulation with 'dplyr' - filter") # Run this line (Strg-Enter) to show info
...
info("'ggplot2': Some axes features") # Run this line (Strg-Enter) to show info
```{r "2_3__6"}
...
info("Remark on graphical check") # Run this line (Strg-Enter) to show info
...
By running the same kernel regression like in part *a)* of this exercise for each of the covariates (over the marginality index) this result can be reproduced. Without any confidence intervals, the plot of the lines of fitted values from the different kernel regressions looks as follows:
...
info("Data manipulation with 'dplyr' - select") # Run this line (Strg-Enter) to show info
```{r "_2_4___3"}
...
info("Remark on the column covariate in the final data frame") # Run this line (Strg-Enter) to show info
...
info("Remark on the functional form") # Run this line (Strg-Enter) to show info
...
info("effects_tobit_dM_AER(tobit, data, p_lower, p_upper) - Introduction and Code") # Run this line (Strg-Enter) to show info
...
library(gridExtra)
#Show both plots
p_sharp_simple_effectP
p_sharp_simple_effectE
```
info("'ggplot2' - geom_bar, coord_flip and theme") # Run this line (Strg-Enter) to show info
...
```
info("Remark on the estimated marginal effects") # Run this line (Strg-Enter) to show info
...
info("Remark on the corresponding results in the paper") # Run this line (Strg-Enter) to show info
- - -
...
info("Skipping another step on the way to the most accurate estimation") # Run this line (Strg-Enter) to show info
- - -
...
info("Instrumental Variable Estimation") # Run this line (Strg-Enter) to show info
...
```{r "2_6__5",results='asis', fig.height=10}
...
p3
```
info("Remark on the other specifications") # Run this line (Strg-Enter) to show info
...
info("Newey_SB_ivtobit - Code and introduction into Newey two-step estimator and Smith-Blundell procedure") # Run this line (Strg-Enter) to show info
...
info("Remark on the results from Newey_SB_ivtobit") # Run this line (Strg-Enter) to show info
...
info("Remark on the results (1)") # Run this line (Strg-Enter) to show info
...
info("Remark on the baseline probability of deforestation") # Run this line (Strg-Enter) to show info
...
info("Robustness tests") # Run this line (Strg-Enter) to show info
- - -
...
*Jacob and Lefgren (2004, Chapter V)* treat the interpretation for the case using an RD design with excluded instruments that allow a fast but *continuous* increase in some marginal zone - like in this exercise. They come to the conclusion that the area for which the measured effect is valid is a bit larger, but still limited to a value of the running variable close to the cutoff.
...
info("Remark on the name PROGRESA") # Run this line (Strg-Enter) to show info
...
info("Comment on household data set") # Run this line (Strg-Enter) to show info
...
We will have a look at the first two groups of variables in this exercise, leaving the consumption and production variables to the *Exercises 3.2* and *3.3*.
...
ggplot(d, aes(x=poor, group=treatment, fill = treatment)) +
  ...
  ylab("Frequency")
```
info("ifelse") # Run this line (Strg-Enter) to show info
info("ggplot2: Graphs for different groups of observations - in the same figure") # Run this line (Strg-Enter) to show info
...
Please use the data set *d* again for plotting. A template of the code is already given.
info("ggplot2: Graphs for different groups of observations - in different figures") # Run this line (Strg-Enter) to show info
```{r "3_1__4",fig.width=12}
...
info("Remark on dens_road_inv") # Run this line (Strg-Enter) to show info
```{r "3_1__6"}
...
info("Background on possible production mechanisms") # Run this line (Strg-Enter) to show info
...
info("Data manipulation with 'dplyr' - overview") # Run this line (Strg-Enter) to show info
...
- - -
#! addon__quiz__Spillover vs. treatment
- - -
...
info("Interaction terms and crosses in lm/felm formula") # Run this line (Strg-Enter) to show info
...
Here, $\widehat{\beta_3}$ is the estimated effect for the case in which the inverse road density is close to $0$ (very much infrastructure) and $\widehat{\beta_7}$ describes how the effect changes with the inverse road density. *(Cf. paper, p.428)* This can be shown by using again the definition of the DD estimator introduced in *Exercise 3.2* (difference in differences in expectation values). *If you are interested in the according derivation, have a look at the 'info' box.*

info("DD estimator varying in R") # Run this line (Strg-Enter) to show info
...
info("Effect of inverse road density on spillover effect") # Run this line (Strg-Enter) to show info
...
At the end of the last exercise we have stated a functional chain linking treatment and deforestation. This approach, which states a demand-driven increase in deforestation, seems to be plausible in so far as we could find an increase in the consumption of land-intensive goods (see *Exercises 3.2, 3.4*) and as the size of this increase supports the hypothesis that consumption is a major mechanism linking treatment and deforestation (see plausibility check in *Exercise 3.2*). However, as the corresponding local production increase is heterogeneous (see *Exercise 3.4*), we have to check furthermore whether we can also find **heterogeneity with respect to road density in the treatment effect on local deforestation**. If this was not the case, we would have to give up the current explanation.
...
info("Data manipulation with 'dplyr' - arrange") # Run this line (Strg-Enter) to show info
...
info("Some helpful commands") # Run this line (Strg-Enter) to show info
```{r "3_5__2"}
...
info("Remark on the results (2)") # Run this line (Strg-Enter) to show info
...
info("Remark on the results (3)") # Run this line (Strg-Enter) to show info
...
- - -
#! addon__quiz__Prediction 1
- - -
#! addon__quiz__Prediction 2
- - -
#! addon__quiz__Prediction 3
- - -
...
info("Treatment saturation in rings") # Run this line (Strg-Enter) to show info
...
info("Estimation strategy and results") # Run this line (Strg-Enter) to show info
...
Even though it might be difficult to conduct such studies based on data from the past, like on Oportunidades or on Progresa, one could also use data from the present as the program (named Prospera) still exists today (as of 2016). The existence of the program - albeit with adapted rules - also shows that even about 20 years after the initiation of the program it is still a current issue. *(Cf. [Prospera homepage (2016)](http://www.gob.mx/prospera "http://www.gob.mx/prospera"))*
...
* Barbier, E. B. & J. C. Burgess (2001): The Economics of Tropical Deforestation. *Journal of Economic Surveys, 15*(3), pp. 413-433.

* Blundell, R. & A. Duncan (1998), Kernel Regression in Empirical Microeconomics. *The Journal of Human Resources, 33*(1), pp. 62-87.
...
* --------- (2002): Determinants of Deforestation and the Economics of Protection: An Application to Mexico. *Journal of Agricultural Economics 84*(4), pp. 943-960.

* Foster, A. D. & M. R. Rosenzweig (2003): Economic Growth and the Rise of Forests. *The Quarterly Journal of Economics, 118*(2), pp. 601-637.

* Greene, W. (1999): Marginal effects in the censored regression model. *Economics Letters, 64*(1), pp. 43-49. 

* Greene, W. H. (2008): *Econometric Analysis*. 6th Edition, Upper Saddle River, NJ: Pearson Education.

* Hackl, P. (2013): *Einführung in die Ökonometrie*. 2nd Edition, Munich: Pearson Deutschland.

* Härdle, W. & O. Linton (1994): Applied nonparametric methods. In:  Engle, R. F. & D. McFadden, Eds., *Handbook of Econometrics*, Volume 4, Chapter 38, Amsterdam: Elsevier, pp. 2295-2339.
...
* Son, H. H. (2011): *Equity and Well-Being: Measurement and Policy Practice*. Oxon, UK: Routledge and Asian Development Bank. <http://www.adb.org/sites/default/files/publication/29288/equity-well-being.pdf>.
...
* World Bank (1992): World Development Report 1992: Development and the Environment. New York, NY: Oxford University Press. <https://openknowledge.worldbank.org/handle/10986/5975>.
...
* CRAN Reference manual for package 'car' (2016): <https://cran.r-project.org/web/packages/car/car.pdf>, retrieved September 22, 2016.

* Prospera homepage (2016): <http://www.gob.mx/prospera>, retrieved July 5, 2016.
...
* Weier, J. & D. Herring (2000): Measuring Vegetation (NDVI & EVI). *NASA Earth Observatory*. August 30, 2000. <http://earthobservatory.nasa.gov/Features/MeasuringVegetation/printall.php>.

## R and R Packages:
* 'AER': Kleiber, C. & A. Zeileis (2008): *Applied Econometrics with R*. New York, NY: Springer-Verlag. <http://CRAN.R-project.org/package=AER>.

* 'car': Fox, J. & S. Weisberg (2011): *An R Companion to Applied Regression*. 2nd Edition, Thousand Oaks, CA: Sage Publications. <http://socserv.socsci.mcmaster.ca/jfox/Books/Companion>.
...
The following points describe in detail how I obtain the different input parameters according to the description in Newey (1987). I should remark here that I focus on cases with only one endogenous explanatory variable, so $Y_t$ is a scalar. **My implementation** is analogous to the estimation steps on page 12 in the STATA ivprobit help file - adapted to the IV Tobit framework. *(Cf. [Stata manual, command ivtobit (2016, p.10)](http://www.stata.com/manuals13/rivtobit.pdf "http://www.stata.com/manuals13/rivtobit.pdf"))*
...
info("Remark on suitability of the model") # Run this line (Strg-Enter) to show info
